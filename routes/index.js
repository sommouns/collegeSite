...
router.get('/', function (req, res) {
    var login_num = 0;
    if (req.query.login_num) var login_num = req.query.login_num;
...
router.get(
    ...
    function (req, res, next) {
        ...
        client = usr.connect();
        usr.selectLatestResS(client, '网络安全', function (result) {
            for (var o in result) {
                var time = JSON.stringify(result[o].news_date);
                time = time.substring(1, 11);
                result[o].news_date = time;
...
router.get(
    ...
    function (req, res, next) {
        ...
        client = usr.connect();
        usr.selectLatestResS(client, '计算机科学与技术', function (result) {
            for (var o in result) {
                var time = JSON.stringify(result[o].news_date);
                time = time.substring(1, 11);
                result[o].news_date = time;
...
router.get(
    ...
    function (req, res, next) {
        ...
        client = usr.connect();
        usr.selectLatestResS(client, '自动化', function (result) {
            for (var o in result) {
                var time = JSON.stringify(result[o].news_date);
                time = time.substring(1, 11);
                result[o].news_date = time;
...
router.get(
    ...
    function (req, res, next) {
        ...
        client = usr.connect();
        usr.selectLatestResS(client, '机械工学', function (result) {
            for (var o in result) {
                var time = JSON.stringify(result[o].news_date);
                time = time.substring(1, 11);
                result[o].news_date = time;
...
router.get(
    ...
    function (req, res, next) {
        ...
        client = usr.connect();
        usr.selectLatestResS(client, '电子通信工程', function (result) {
            for (var o in result) {
                var time = JSON.stringify(result[o].news_date);
                time = time.substring(1, 11);
                result[o].news_date = time;
...
router
    ...
    .get(function (req, res) {
        ...
        var hint = '';
        if (req.query.hint == 1) hint = '用户名或密码错误，请重试';
        if (req.query.hint == 2) hint = '暂无权限访问，请先登录！';
        res.render('admin', {
            title: '管理员登录',
            test: res.locals.islogin,
            hint: hint,
...
router.get('/admin_view_remark', function (req, res, next) {
    console.log('admin_view_remark');
    res.render('admin/view_remark', {});
});
...
router.get('/getnewsinfo', function (req, res, next) {
    ...
    usr.selectNews(client, id, function (result) {
        ...
        switch (result[0].news_sort) {
            case 'sort1':
                result[0].news_sort = '头条热点';
                break;
            case 'sort2':
                result[0].news_sort = '学院政策';
                break;
            case 'sort3':
                result[0].news_sort = '学术论文';
                break;
            case 'sort4':
                result[0].news_sort = '学院竞赛';
                break;
            case 'sort5':
                result[0].news_sort = '公告栏';
                break;
        }
        res.json(result);
    });
...
//删除服务器处remark
router.post('/dangerwaytodothis_remark', function (req, res, next) {
    var id = req.body.id;
    console.log('id=:' + id);
    client = usr.connect();
    usr.deleteRemark(client, id, function (err) {
        console.log('err from router when deleting:' + err);
        res.json(err);
    });
});
...
router.post('/admin_update_news_send', function (req, res, next) {
    //发送修改好的新闻
...
router.post('/admin_update_res_send', function (req, res, next) {
    //发送修改好的res
...
router.post('/sendRemark', function(req, res, next) {
    client = usr.connect();
    usr.insertRemark(client, req.session.islogin, req.body.title, req.body.body, function(err) {
        res.json(err);
    })
})
//向服务器添加res
...
router.get('/getnews', function (req, res, next) {
    ...
    usr.selectNewsList(client, function (result) {
        for (var o in result) {
            switch (result[o].news_sort) {
                case 'sort1':
                    result[o].news_sort = '头条热点';
                    break;
                case 'sort2':
                    result[o].news_sort = '学院政策';
                    break;
                case 'sort3':
                    result[o].news_sort = '学术论文';
                    break;
                case 'sort4':
                    result[o].news_sort = '学院竞赛';
                    break;
                case 'sort5':
                    result[o].news_sort = '公告栏';
                    break;
            }
            var time = JSON.stringify(result[o].news_date);
...
router.get('/getRemarks', function (req, res, next) {
    client = usr.connect();
    result = null;
    usr.getRemarkList(client, function (result) {
        for (var o in result) {
            var time = JSON.stringify(result[o].date);
            result[o].date = time.substring(1, 11);
        }
        console.log(result);
        res.json(result);
    });
