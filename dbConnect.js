...
function insertFun(client, username, password, callback) {
    client.query(
        'insert into userinfo value(NULL,?,?)',
        [username, password],
        function (err, result) {
            if (err) {
...
function insertNews(client, news, callback) {
    client.query(
        'insert into news value(NULL,?,?,?,?,?,?)',
        [news.title, news.date, news.source, news.sort, news.content, 'on'],
...
function insertRes(client, news, callback) {
    client.query(
        'insert into research value(NULL,?,?,?,?,?,?)',
        [news.title, news.date, news.source, news.sort, news.content, 'on'],
        function (err, results) {
            if (err) {
                console.log('error from DB:' + err.message);
...
function insertDoc(client, doctor, callback) {
    client.query(
        'insert into teacherinfo(name,major,birthplace, birthdate, graduates,title,degree,job,workplace,worktime,info01,info02,info03,info04,info05,info06,info07,info08,info09,info10,avatar) value (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [
            doctor.name,
            doctor.major,
            doctor.birthplace,
...
function selectNewsList(client, callback) {
    ...
    client.query(
        'select id,news_title,news_date,news_source,news_sort,status from news',
        function (err, results) {
            if (err) {
                throw err;
                console.log('err form DB:' + err);
            }
            console.log('results:' + results);
            callback(results);
        }
...
function selectResList(client, callback) {
    ...
    client.query(
        'select id,news_title,news_date,news_source,news_sort,status from research',
        function (err, results) {
            if (err) {
                throw err;
                console.log('err form DB:' + err);
...
function selectExpList(client, callback) {
    ...
    client.query(
        'select id,name,major,title,workplace,status from teacherinfo',
        function (err, results) {
            if (err) {
                throw err;
                console.log('err form DB:' + err);
...
function UpdateNews(client, essay, callback) {
    //修改数据库里的文章
    client.query(
        'update news set news_title= ?,news_date= ?,news_source=?,news_sort = ?,news_body =? where id ="' +
            essay.id +
            '"',
        [
            essay.news_title,
...
function UpdateRes(client, essay, callback) {
    //修改数据库里的res
    client.query(
        'update research  set news_title= ?,news_date= ?,news_source=?,news_sort = ?,news_body =? where id ="' +
            essay.id +
            '"',
        [
            essay.news_title,
...
function SetNewsStatus(client, id, status, callback) {
    //设置新闻状态
    client.query(
        'update news set status = "' + status + '" where id ="' + id + '"',
        function (err, results) {
            if (err) throw err;
            console.log('results:' + results);
            callback(err);
...
function SetResStatus(client, id, status, callback) {
    //设置res状态
    client.query(
        'update research set status = "' + status + '" where id ="' + id + '"',
        function (err, results) {
            if (err) throw err;
            console.log('results:' + results);
            callback(err);
...
function selectLatest(client, sort, callback) {
    //专家详情
    client.query(
        'select id,news_title,news_date from news where news_sort ="' +
            sort +
            '" and status = "on" order by id desc',
        function (err, results) {
            if (err) throw err;
...
function selectLatestRes(client, callback) {
    //res
    client.query(
        'select id,news_title,news_date,news_sort from research where status = "on" order by id desc',
        function (err, results) {
            if (err) throw err;
...
function selectLatestResS(client, sort, callback) {
    //res
    client.query(
        'select id,news_title,news_date,news_sort from research where news_sort = "' +
            sort +
            '" and status = "on" order by id desc',
        function (err, results) {
            if (err) throw err;
...
function selectLatestExp(client, callback) {
    //res
    client.query(
        'select id,name,avatar from teacherinfo where status = "on" order by id desc',
        function (err, results) {
            if (err) throw err;
            console.log('results res:' + results);
            callback(results);
